...
```{r tl1, fig.width=8, fig.height=4, fig.cap="Timeline of prequel techniques.", out.width="100%"}
events %>% 
...
```{r trap1, fig.cap='(ref:trap)', out.width="100%"}
...
```{r hist1, fig.width=8, fig.height=8, fig.cap='(ref:hist1c)', out.width="100%"}
method <- unnest_cat(prequel, method, other_cols = "species")
pubs_per_year(method, facet_by = "method", fill_by = "species", n_top_fill = 7) +
   labs(title = "Number of publications over time", y = NULL) +
...
ish <- prequel %>% 
  ...
  unnest_cat(species, other_cols = c("organ", "n_items", "still_available"))
```
...
```{r items, fig.width=8, fig.height=8, fig.cap='(ref:itemc)', out.width="100%"}
pubs_per_year(prequel, facet_by = "item_type", n_top = 5, 
                      fill_by = "species") +
   geom_vline(xintercept = ymd("2002-12-05"), color = "gray70") +
   geom_text(data = item_annot, aes(label = label), x = ymd("2001-08-01"), y = 12) +
...
```{r species, fig.width=6, fig.height=4, fig.cap="Number of (WM)ISH publications per species."}
pubs_per_cat(ish, species) +
...
```{r tl2, fig.width=10, fig.height=6, fig.cap="Timeline of the first (WM)ISH databases for each species for which such databases are available, as well as some notable databases.", out.width="100%"}
events %>% 
  filter(category == "ISH atlas") %>% 
  plot_timeline(c(0.5, -0.5, -0.85, 0.7, -0.25, 1, -0.5, 0.3, -0.8, 0.5, -1, 
                  0.7, -0.25, 0.5, 
...
```{r}
mm_ish_tissues <- ish %>% 
    filter(str_detect(species, "Mus musculus"), !is.na(organ)) %>% 
    unnest_cat(organ, other_cols = "n_items")
mm_ish_tissues <- mm_ish_tissues %>% 
  left_join(mmMale_key[, c("organ", "type")], by = "organ")
```

```{r}
organs_npubs <- mm_ish_tissues %>% 
    count(organ, type, name = "value") %>% 
    gganatogram(organism = "mouse", fill = "value") +
    coord_equal() +
    theme_void() +
    scale_fill_distiller(palette = "Blues", direction = 1, 
                         name = "# publications", breaks = scales::breaks_width(4))
```

```{r}
organs_ngenes <- mm_ish_tissues %>% 
   group_by(organ, type) %>% 
   summarise(value = max(n_items, na.rm = TRUE)) %>% 
   gganatogram(organism = "mouse", fill = "value") +
   coord_equal() +
   theme_void() +
   scale_fill_distiller(palette = "PuRd", direction = 1, trans = "log10",
                        name = "# genes\n(log scale)")
```
...
```{r}
species_pie <- prequel %>% 
  mutate(species = fct_lump_min(species, 5)) %>% 
  count(species) %>% 
...
```{r}
n_species <- sum(species_pie$species != "Other")
```

```{r}
pal_use <- c(setNames(ditto_colors[seq_len(n_species)],
                      species_pie$species[species_pie$species != "Other"]),
             Other = "gray50")
```
...
ggplot(species_pie, aes(x = "", y = n, fill = species)) +
   geom_col(position = "stack", color = "gray50", linewidth = 0.3, show.legend = FALSE,
            width = 1) +
...
```{r anat1, fig.width=6, fig.height=4, fig.cap="A) Number of mouse publications per organ for (WM)ISH atlases (including FISH). B) Maximum number of genes in atlases for each organ, as of publication of the paper about the atlases. The color is in log scale to improve dynamic range."}
organs_npubs + organs_ngenes +
    plot_layout(nrow = 1) +
...
```{r tech1, fig.width=6, fig.height=3, fig.cap="Number of prequel publications per technique."}
pubs_per_cat(method, method) +
   labs(x = "Technique")
```
...
```{r dbs, fig.width=6, fig.height=4, fig.cap="Number of extant spatial gene expression databases per species."}
dbs %>% 
...
city_gc <- geocode_inst_city(prequel, cache_location = here())
...
```{r prequel-world, fig.width=10, fig.height=5, fig.cap="Number of prequel publications per city around the world, with top contributing institutions labeled.", out.width="100%"}
pubs_on_map2(prequel)
```

```{r prequel-us, fig.width=9, fig.height=6, fig.cap="Number of prequel publications in the US and Canada, with top contributing institutions labeled.", out.width="100%"}
pubs_on_map2(prequel, zoom = "usa")
```

```{r prequel-europe, fig.width=6, fig.height=6, fig.cap="Number of prequel publications in western Europe, with top contributing institutions labeled."}
pubs_on_map2(prequel, zoom = "europe")
```

```{r prequel-ne-asia, fig.width=8, fig.height=6, fig.cap="Number of prequel publications in northeast Asia, with top contributing institutions labeled.", out.width="100%"}
pubs_on_map2(prequel, zoom = "ne_asia")
```
...
```{r map-species, fig.width=10, fig.height=12.5, fig.cap="Number of prequel publications per city broken down by species. Gray points are the overall number as a reference of contributions from each city and region.", out.width="100%"}
pubs_on_map2(prequel, facet_by = "species", n_top = 8, ncol = 2, n_label = 3) +
...
```{r map-tech, fig.width=10, fig.height=10, fig.cap="Number of prequel publications per city broken down by technique. Gray points are the overall number as a reference of contributions from each city and region.", out.width="100%"}
prequel %>% 
    unnest_cat(method, other_cols = "short_name") %>% 
    pubs_on_map2(facet_by = "method", ncol = 2, n_label = 3) +
    theme(legend.position = "right")
```
