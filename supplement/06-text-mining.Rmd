...
```{r lcm-map, fig.width=10, fig.height=6, fig.cap="Geographic distribution of LCM transcriptomics research, with top 10 cities labeled. Number of publications is binned over longitude and latitude.", out.width="100%"}
...
```{r topics, fig.width=8, fig.height=10, fig.cap="Top words for each of the 50 topics, ordered by expected topic prevalence and showing top 5 words contributing to each topic.", out.width="100%"}
...
```{r topic-words, fig.width=12, fig.height=15, fig.cap="Probability of top 10 words in each topic. Open image in new tab to see the text.", out.width="100%"}
...
```{r topic-corr, fig.height=7, fig.width=10, fig.cap="Correlation between topics.", out.width="100%"}
set.seed(0)
...
```{r wbt, fig.width=10, fig.height=10, fig.cap="(ref:wbt-cap)", out.width="100%"}
plot_facets_color(p1, type, pal_strip)
```

```{r}
foo <- ac_dfm2_tidy %>% 
  filter(term == "time_pcr") %>% 
  slice_max(count, n = 10) %>% 
  left_join(lcm_abstracts, by = c("document" = "title")) %>% 
  arrange(desc(count))
```
...
```{r word-heat, fig.width=8, fig.height=6, fig.cap="Heat map clustering changes in word frequency over time. The rows of the matrix are normalized, only showing trend rather than frequency.", out.width="100%"}
...
```{r topic-trends, fig.width=10, fig.height=12, fig.cap="(ref:tt-cap)", out.width="100%"}
...
```{r topic-count, fig.width=10, fig.height=12, fig.cap="(ref:topic-count-cap)", out.width="100%"}
...
```{r topic-corr-trend, fig.height=7, fig.width=10, fig.cap="Correlation between topics colored by both broad categories of the topics and whether its proportion increased, decreased, or did not significantly change (n.s.).", out.width="100%"}
set.seed(0)
ggraph(topic_cor_graph, layout = "fr") +
  geom_edge_fan(color = "gray50") +
...
```{r topic-loc, fig.width=10, fig.height=6, fig.cap="Cities associated with topics (p < 0.005) shown on a map.", out.width="100%"}
ggplot() +
...
```{r city-pe, fig.width=6, fig.height=4, fig.cap="(ref:city-pe-cap)"}
effs_df_loc %>% 
...
```{r}
shapes <- c(16,17,15,18,4,5,8,9,1,0)
```

```{r pc12, fig.width=9, fig.height=6, fig.cap="Projection of word embeddings into the first 2 PCs. Each point is a word occuring over 30 times in the corpus. Not all words are labeled to avoid overlaps in the labels. Words and points are colored by Louvain clusters.", out.width="100%"}
...
```{r pc34, fig.width=9, fig.height=6, fig.cap="Projection of word embeddings into the 3rd and 4th PCs.", out.width="100%"}
...
```{r umap, fig.width=13, fig.height=13, fig.cap="UMAP projection of word embeddings. Zoom in if reading the PDF version of this book.", out.width="100%"}
