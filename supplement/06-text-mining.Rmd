...
```{r lcm-map, fig.width=12, fig.height=7, fig.cap="Geographic distribution of LCM transcriptomics research, with top 10 cities labeled. Number of publications is binned over longitude and latitude."}
...
```{r topic-corr, fig.height=8, fig.width=12, fig.cap="Correlation between topics."}
set.seed(0)
...
p1 <- ggplot(word_plt, aes(time_floor, prop)) +
   ...
   facet_wrap(~ word, ncol = 5, labeller = as_labeller(facet_labels))
```
...
```{r wbt, fig.width=12, fig.height=8, fig.cap="(ref:wbt-cap)"}
...
```{r topic-trends, fig.width=12, fig.height=10, fig.cap="(ref:tt-cap)"}
...
p3 <- ggplot(year_counts_plt, aes(date_bin, n)) +
  ...
  geom_label(data = topic_annot, aes(x = mid_date, y = max_y2, 
             ...
             alpha = 0.7, vjust = 1, show.legend = FALSE) +
  scale_color_brewer(drop = FALSE, palette = "Dark2", name = "Category") +
  facet_wrap(~ topic, ncol = 5, labeller = as_labeller(facet_labels2)) +
...
```{r topic-count, fig.width=12, fig.height=10, fig.cap="Number of abstracts with each topic whose proportions changed the most in time. Gray ribbon is the 95% CI of the line fitted to the count per year."}
...
```{r topic-corr-trend, fig.height=8, fig.width=12, fig.cap="Correlation between topics colored by both broad categories of the topics and whether its proportion increased, decreased, or did not significantly change (n.s.)."}
set.seed(0)
ggraph(topic_cor_graph, layout = "fr") +
  geom_edge_fan(color = "gray50") +
...
```{r topic-loc, fig.width=12, fig.height=8, fig.cap="Cities associated with topics (p < 0.005) shown on a map."}
ggplot() +
...
```{r}
wf <- quanteda::textstat_frequency(lcm_dfm2)
...
```{block2, note-text3, type='rmdnote', include=TRUE}
...
```{r umap, fig.width=13, fig.height=13, fig.cap="UMAP projection of word embeddings."}
