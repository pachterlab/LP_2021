...
Some microdissection methods can only be used in a regular grid, such as Tomo-seq, while some can be used either in a regular grid or in targeted ROIs, such as LCM and GeoMX DSP. Some are primarily used for ROIs, such as Niche-seq.

```{r}
ms <- read_metadata("Microdissection")
...
### Summary
```{r}
nms <- c("Microdissection", "smFISH", "NGS wo MD", "ISS", "No imaging")
...
```{r smfish-broken-y, fig.width=9, fig.height=6, fig.cap="Like the previous figure, but more zoomed in to datasets with fewer than 10,000 genes.", out.width="100%"}
smfish %>% 
  filter(!is.na(n_genes)) %>% 
...
After its inception, HybISS became part of a single cell atlas of the developing mouse nervous system [@Manno2021]. This atlas is mostly scRNA-seq data, but 119 genes were stained with HybISS to validate secondary organizers discovered via scRNA-seq.
