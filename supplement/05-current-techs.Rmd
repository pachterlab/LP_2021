...
Some methods can only be used in a regular grid, such as Tomo-seq, while some can be used either in a regular grid or in targeted ROIs, such as LCM and GeoMX DSP (Figure \@ref(fig:targeted). Some are primarily used for ROIs, such as Niche-seq. Sometimes an ROI in the section may be chosen, which is then divided into smaller regular parts, in between targeted and untargeted. 

```{r}
ms <- read_metadata(c("Microdissection", "Other"), update = TRUE)
...
```{r smfish-trend, fig.width=6, fig.height=6, fig.cap="Number of publications over time, broken down by technique type. Preprints are included, and the gray histogram in the background is the overall trend of all smFISH based techniques. Bin width is 365 days."}
...
ggplot(smfish_methods_cnt, aes(factor(n, levels = as.character(7:1)))) +
   geom_bar() +
...
```{r array-map, fig.height=13.75, fig.width=10, fig.cap="Cities and institutions using ST and Visium. Preprints are included.", out.width="100%"}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", n_top = 2,
            n_label = 5, ncol = 1)
```

```{r array-us, fig.height=9, fig.width=6, fig.cap="Cities and institutions using ST and Visium around continental US. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", 
            zoom = "usa",
            n_top = 2, n_label = 5, ncol = 1)
```

```{r array-europe, fig.height=10, fig.width=6, fig.cap="Cities and institutions using ST and Visium in western Europe. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", 
            zoom = "europe",
            n_top = 2, n_label = 5, ncol = 1)
```
...
1. In ROI based studies, typically only a small number of ROIs are used. It can be labor intensive to study the histological staining to manually select ROIs.
...
2. Again, existing well-established commercial platforms and core facilities for LCM and GeoMX DSP, though this might change with MERSCOP, Rebus Esper, Molecular Cartography, and commercial probe panels.
...
In the previous subsection, cons of ROI based methods compared to NGS wo MD would be the pros of NGS wo MD compared to ROI based methods. Compared to smFISH and ISS:
