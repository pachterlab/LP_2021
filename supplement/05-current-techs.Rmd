...
#### Amplification
The miniscule amount of transcripts from microdissected tissues, which can be single cells, need to be amplified to be detected by microarray or RNA-seq. Indeed, RNA amplification is a part of one of the most prevalent topics in LCM related search results (\@ref(fig:topics), \@ref(fig:topic-words)).
...
(ref:smfish-lm-cap) Number of genes per datasets in each study, over time. Gray ribbon is 95% confidence interval (CI). The points are translucent; more opaque points are multiple datasets from the same study. Here a "dataset" means either a different tissue or cell type or a separate experiment profiling a different number or set of genes in the same study. One dataset can involve multiple sections and individuals.
