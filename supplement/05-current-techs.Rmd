...
```{r smfish-lm-gene, fig.width=9, fig.height=6, fig.cap="(ref:smfish-lm-cap)"}
...
```{r, results='markup'}
summary(lm(n_genes ~ date_published, data = smfish))
```
...
```{r}
smfish <- smfish %>% 
  mutate(mask = case_when(n_genes > 5000 ~ "1",
                          #n_genes > 1000 ~ "2",
                          TRUE ~ "2"))
```

```{r smfish-broken-y, fig.width=9, fig.height=6, fig.cap="Like the previous figure, but more zoomed in to datasets with fewer than 10,000 genes."}
smfish %>% 
  filter(!is.na(n_genes)) %>% 
  ggplot(aes(date_published, n_genes)) +
  geom_point(aes(color = method), alpha = 0.5) +
  geom_text_repel(aes(label = method, color = method), segment.alpha = 0.5,
                  max.overlaps = 20, min.segment.length = 0.22) +
  geom_smooth(method = "lm", 
              data = smfish %>% filter(mask == "2", !is.na(n_genes))) +
  labs(x = "Date published", 
       title = "Number of genes per dataset",
       y = NULL) +
  theme(legend.position = "none", strip.text = element_blank(),
        strip.background = element_blank()) +
  facet_grid(rows = vars(mask), space = "free_y", scales = "free_y") +
  scale_y_continuous(breaks = scales::breaks_width(1000),
                     expand = expansion(add = c(150,150)))
```
...
```{r smfish-lm-cell, fig.width=9, fig.height=6, fig.cap="Total number of cells per study profiled by smFISH based techniques over time."}
...
```{r smfish-langs, fig.width=6, fig.height=4, fig.cap="Number of publications using smFISH based techniques that used each of the 5 most common programming languages. Each icon stands for 2 publications."}
