...
```{r lcm, fig.cap="(ref:lcm-cap)", out.width="100%"}
knitr::include_graphics("LCM.png")
```
...
```{r voxelation, fig.cap="(ref:vox)", out.width="190px", out.height="318px"}
knitr::include_graphics("voxelation.png")
```
...
```{r tomo, out.width="300px", out.height="314px", fig.cap="Tomo-seq, here showing C. elegans."}
knitr::include_graphics("tomo.png")
```
...
```{r niche, out.width="375px", out.height="300px", fig.cap="Niche-seq schematics. Green: cells with photoactivated PA-GFP."}
knitr::include_graphics("niche.png")
```
...
```{r dsp, fig.cap="(ref:dsp)", out.width="100%"}
knitr::include_graphics("dsp.png")
```
...
### Summary
```{r}
nms <- c("Microdissection", "smFISH", "Array", "ISS", "No imaging")
current <- read_metadata(nms, update = TRUE) %>% 
    select(date_published:journal, country:year, sheet) %>% 
    distinct()
```

```{r categories, fig.width=6, fig.height=3, fig.cap="Number of publications per category of techniques in the current era. Non-curated LCM literature is excluded."}
...
```{r smfish, out.width="441px", out.height="189px", fig.cap="(ref:smfish-cap)"}
knitr::include_graphics("smfish1998.png")
```
...
```{r hb, fig.cap="(ref:hb-cap)", out.width="90%"}
knitr::include_graphics("historical_barcoding.png")
```
...
```{r seqfish, fig.cap="(ref:seqfish-cap)", out.width="100%"}
knitr::include_graphics("seqfish.png")
```
...
```{r merfish, fig.cap="(ref:merfish-cap)", out.width="100%"}
knitr::include_graphics("merfish.png")
```
...
```{r sf-plus, fig.cap="Schematic of seqFISH+.", out.width="100%"}
knitr::include_graphics("seqfish-plus.png")
```
...
```{r split-fish, fig.cap="Schematic of split-FISH.", out.width="100%"}
knitr::include_graphics("split-fish.png")
```
...
```{r bdna, out.width="100%", fig.cap="Schematic of bDNA. The Z probes are specific to RNAscope, but the other parts are generic to bDNA."}
knitr::include_graphics("bdna.png")
```
...
```{r rca, out.width="100%", fig.cap="Schematic of RCA, here shown with target priming though a separate primer can also be used. Red segment is the gene barcode."}
knitr::include_graphics("rca.png")
```
...
```{r hcr, out.width="100%", fig.cap="Schematic of HCR, showing 3 cycles, but this can continue indefinitely until H1 and H2 are exhausted. Arrow shows 5' to 3' direction."}
knitr::include_graphics("hcr.png")
```
...
```{r exm, out.width="100%", fig.cap="Schematic of expansion microscopy."}
knitr::include_graphics("exm.png")
```
...
```{r records-gene, fig.width=6, fig.height=4, fig.cap="Record number of genes per dataset quantified by smFISH based techniques over time."}
ggplot(max_genes_time, aes(date_published, record_gene)) +
  geom_step() +
  scale_y_log10() +
...
```{r records-cell, fig.width=6, fig.height=4, fig.cap="Record total number of cells per study profiled by smFISH based techniques over time."}
ggplot(max_cell_time, aes(date_published, record_cell)) +
  geom_step() +
  scale_y_log10() +
  annotation_logticks(sides = "l") +
...
```{r smfish-lm-gene, fig.width=9, fig.height=6, fig.cap="Number of genes, averaged over datasets in each study, over time. The studies profiling 10,000 genes are excluded to better show the trend among more ordinary studies."}
mean_genes2 <- mean_genes %>% 
  filter(n_genes < 1500)
ggplot(mean_genes2, aes(date_published, n_genes)) +
  ...
  theme(legend.position = "none") +
    coord_cartesian(ylim = c(0, max(mean_genes2$n_genes)))
```

```{r smfish-lm-gene2, fig.width=9, fig.height=6, fig.cap="Like the previous figure, but showing the outliers."}
ggplot(mean_genes, aes(date_published, n_genes)) +
    geom_point(aes(color = method)) +
    geom_text_repel(aes(label = method, color = method), segment.alpha = 0.5) +
    geom_smooth(method = "lm") +
    labs(x = "Date published", title = "Mean number of genes per study per method",
         y = NULL) +
    theme(legend.position = "none") +
    coord_cartesian(ylim = c(0, max(mean_genes$n_genes)))
```
...
```{r, results='markup'}
summary(lm(n_genes ~ date_published, data = mean_genes))
```
...
```{r smfish-lm-cell, fig.width=9, fig.height=6, fig.cap="Total number of cells per study profiled by smFISH based techniques over time. Studies with more than 200,000 cells are excluded to better show the trend among more ordinary studies. "}
...
ggplot(sum_cells2, aes(date_published, n_cells)) +
  ...
  geom_text_repel(aes(label = method, color = method), segment.alpha = 0.5) +
  geom_smooth(method = "lm") +
  labs(x = "Date published", title = "Total number of cells per study", 
       y = NULL) +
  theme(legend.position = "none") +
...
```{r smfish-lm-cell2, fig.width=9, fig.height=6, fig.cap="Like the previous figure, but showing the outliers."}
ggplot(sum_cells, aes(date_published, n_cells)) +
    geom_point(aes(color = method)) +
    geom_text_repel(aes(label = method, color = method), segment.alpha = 0.5) +
    geom_smooth(method = "lm") +
    labs(x = "Date published", title = "Total number of cells per study", y = NULL) +
    theme(legend.position = "none") +
    coord_cartesian(ylim = c(0, max(sum_cells$n_cells)))
```
...
```{r, results='markup'}
summary(lm(n_cells ~ date_published, data = sum_cells))
```
...
```{r smfish-trend, fig.width=6, fig.height=6, fig.cap="Number of publications over time, broken down by technique type. Preprints are included, and the gray histogram in the background is the overall trend of all smFISH based techniques."}
pubs_per_year(smfish_pubs, facet_by = "method2", sort_by = "count", 
                       preprints = TRUE) +
   labs(y = NULL, title = "Number of publications over time")
```

```{r}
city_gc2 <- geocode_inst_city(smfish_pubs, cache_location = here())
...
```{r smfish-n-insts, fig.width=6, fig.height=3, fig.cap="Number of techniques that have been used by each number of institutions; most techniques have only been used by 1 institution, i.e. the institution of origin."}
ggplot(smfish_methods_cnt, aes(factor(n, levels = as.character(5:1)))) +
   geom_bar() +
...
smfish_methods2 <- smfish_methods %>% 
  mutate(method2 = case_when(
    ...
    str_detect(method, "Hyb?RISS") ~ "Hyb(R)ISS",
    TRUE ~ "Other"
  )) %>% 
...
```{r smfish-map, fig.width=9, fig.height=6, fig.cap="(ref:smfish-map-cap)"}
pubs_on_map(smfish_methods2, city_gc = city_gc2, facet_by = "method2", 
            ncol = 2) +
...
```{r smfish-langs, fig.width=6, fig.height=4, fig.cap="Number of publications using smFISH based techniques that used each of the 5 most common programming languages. Each icon stands for 5 publications."}
pubs_per_cat(languages, category = language, isotype = TRUE, n_top = 5, 
             img_unit = 2) +
...
```{r}
tab2 <- tibble(Technique = c("HCR-seqFISH", "seqFISH+", "MERFISH", 
                             "ExM-MERFISH", "HybISS", "SCRINSHOT", 
                             "HybRISS", "bDNA-smFISH"),
               Pro = c("Relatively high efficiency (84%), fewer rounds of hybridization, error correction",
                       "Avoids optical crowding, scalable",
                       "High efficiency (95%) with HD4 code, error correction, version 2 relatively fast, scalable",
                       "Avoids optical crowding, clears tissue",
                       "Only 5 probes per gene, applicable to isoform specific exons, padlock probe reduces background, lower magnification when imaging (20x and 40x, while MERFISH uses 60x), can discern SNPs",
                       "Similar to HybISS, but designed for FFPE tissue",
                       "Avoids inefficiency of reverse transcription, better signal to noise ratio and more transcripts detected then HybISS.",
                       "Commercial RNAscope kit, reduces background and amplifies signal, amplified moiety does not grow indefinitely"),
               Con = c("Lower efficiency than MERFISH, time consuming to re-hybridize probes to target after stripping",
                       "Lower efficiency (49%), numerous rounds of hybridization",
                       "Numerous rounds of hybridization, numerous probes requiring long transcripts though this is resolved by bDNA signal amplification",
                       "Each FOV contains less of the original tissue",
                       "Error correction not reported, amplicon takes up space and might drift away",
                       "Same as HybISS",
                       "Padlock probe sequences are proprietary to CARTANA",
                       "Except for bDNA-MERFISH, it has not been used in a highly multiplexed setting"))
```
...
```{r table2}
knitr::kable(tab2, format = out_format, 
             booktabs = knitr::is_latex_output(),
             caption = "Pros and cons of smFISH based techniques.") %>% 
  kable_styling(latex_options = "striped") %>% 
  column_spec(1:3, c("6em", "12em", "12em"))
```
...
```{r fisseq, out.width="100%", fig.cap="Schematic of RCA in FISSEQ."}
knitr::include_graphics("fisseq.png")
```
...
```{r solid, out.width="100%", fig.cap="Schematic of SOLiD sequencing, determining the sequence GATTACA. The rows are arranged in the order of 5' to 3' positions of the first fluorescent probe, but the actual hybridization and ligation can take a different order. As part of the constant region, the 'A' highlighted in red is known."}
knitr::include_graphics("solid.png")
```
...
```{r cpal, out.width="100%", fig.cap="Schematic of cPAL as used in ISS."}
knitr::include_graphics("cpal.png")
```
...
```{r sedal, out.width="100%", fig.cap="Schematic of RCA of SNAIL probe and SEDAL. Also showing error propagation and identification of 2 base encoding. As part of the constant region, the 'G' highlighted in red is known."}
knitr::include_graphics("sedal.png")
```
...
st_city_gc <- geocode_inst_city(array, cache_location = here())
array_pubs <- get_pubs_df(array, other_cols = "short_name")
```

```{r array-map, fig.height=11, fig.width=8, fig.cap="Cities and institutions using ST and Visium. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", n_top = 2, 
            n_label = 5, ncol = 1)
```
...
```{r array-dims, out.width="420px", out.height="525px", fig.cap="Schematic of spot construction and size of array based techniques."}
knitr::include_graphics("fig13a.png")
```

```{r array-umi,out.width="420px", out.height="525px", fig.cap="Barcode and UMI structure and lengths of array based techniques."}
knitr::include_graphics("fig13b.png")
```
...
```{r}
min_spot_size <- array %>% 
    select(spot_diameter_um, date_published) %>% 
    filter(!is.na(spot_diameter_um)) %>% 
    distinct() %>% 
    arrange(date_published, spot_diameter_um) %>% 
    mutate(record_diam = cummin(spot_diameter_um))
```

```{r spot-diam, fig.width=6, fig.height=4, fig.cap="Record spot diameter of array based methods over time."}
ggplot(min_spot_size, aes(date_published, record_diam)) +
    geom_step() +
    scale_y_log10() +
    annotation_logticks(sides = "l") +
    labs(x = "Date", title = bquote(Record~spot~diameter~(mu*m)), y = NULL)
```
