...
(ref:dsp) GeoMX DSP schematics, inspired by figures in [@Merritt2020MultiplexTissue]. Black: transcripts in tissue. Gray: probes. Green: indexing oligo.

```{r dsp, fig.cap="(ref:dsp)", out.width="100%"}
knitr::include_graphics("dsp.png")
```

Another method that spread beyond its institution of origin is the commercial GeoMX DSP from NanoString [@Merritt2020MultiplexTissue], which can be used for both high throughput immunofluorescence and transcript quantification in FFPE tissue sections. While GeoMX DSP does not physically isolate relevant parts of the tissue, it is discussed in this section because like other microdissection based techniques, GeoMX DSP is primarily ROI based, and spatial location is known from selection of the ROI. For transcript quantification, probes are attached to indexing oligos with a UV cleavable linker (Figure \@ref(fig:dsp)). The selected ROI is illuminated by UV to remove the index oligos from the probes. Then the released index oligos are aspirated and quantified with either NGS or NanoString nCounter. This can be repeated for multiple ROIs, which can be a grid for unbiased profiling [@Merritt2020MultiplexTissue]. The probes tile the transcripts, and each probe has a distinct index oligo, so in NGS, each tile is counted separately, enabling isoform quantification [@Merritt2020MultiplexTissue]. The number of genes profiled by GeoMx DSP depends on the gene panel used; the Cancer Transcriptome Atlas panel with over 1800 genes have been used in several studies (e.g. [@Margaroli2021; @Park2021]), and with the human or mouse Whole Transcriptome Atlas (WTA) panel, transcripts of 18190 genes can be quantified, nearly covering the whole transcriptome [@Roberts2021]. In GeoMX WTA, the UV cleaved index oligo must be sequenced with NGS to identify the gene each transcript is from. As pre-defined probes are required, unlike in RNA-seq, novel transcripts cannot be quantified. Ready made probe sets for oncology, immunology, and neuroscience are sold by NanoString [@DSPPanels]. Although GeoMx DSP was published in 2019, it has spread to several different institutions, and has been used on pancreatic ductal adenocarcinoma (PDAC) [@Hwang2020], hepatocellular carcinoma (HCC) [@Sharma2020], reactive lymph nodes [@Tripodo2020], and COVID infected lungs from autopsy [@Park2021; @Butler2021; @Delorey2021a; @Margaroli2021]. 
...
```{r}
ms <- read_metadata("ROI selection", update = TRUE)
ms_pubs <- get_pubs_df(ms, c("type", "method"))
```

```{r targeted, fig.width=6, fig.height=4, fig.cap="Number of studies of each of the three types: targeted, in between, and untargeted, using each microdissection based technique plus GeoMX DSP. Techniques used in less than two studies or two types are lumped into Other."}
ms_pubs %>% 
  mutate(method = fct_lump_min(method, 2) %>% fct_infreq() %>% fct_rev(),
         type = fct_relevel(type, c("targeted", "in between", "untargeted"))) %>% 
  ggplot(aes(y = method, fill = type)) +
  geom_bar(orientation = "y") +
  scale_x_continuous(expand = expansion(c(0, 0.05))) +
  scale_fill_viridis_d(begin = 0.15, end = 0.85, direction = -1,
                       name = "Type", na.value = "gray50") +
  labs(x = "Number of studies", y = "Method")
```
...
ffpe <- read_metadata(nms, update = TRUE) %>% 
  select(title, method, species, tissue, pathological) %>% 
  distinct() %>% 
  filter(!is.na(tissue), !is.na(method)) %>% 
  mutate(is_ffpe = str_detect(tissue, "FFPE"))
ffpe2 <- ffpe %>% 
  mutate(method = fct_lump_min(method, 10) %>% fct_infreq() %>% fct_rev())
```

```{r ffpe, fig.width = 6, fig.height=4, fig.cap="Number of FFPE and frozen section datasets from each current era technique; techniques used in fewer than 10 datasets are lumped into Other. LCM is only for curated LCM literature and does not include all search results in Chapter 6."}
ggplot(ffpe2, aes(y = method, fill = is_ffpe)) +
  geom_bar() +
  scale_x_continuous(expand = expansion(c(0,0.05))) +
  scale_fill_discrete(name = "Is the\nsample\nFFPE") +
  labs(x = "Number of datasets", y = "Method")
```

```{r}
ffpe_pathological <- ffpe %>% 
  filter(species %in% c("Homo sapiens", "Mus musculus"),
         !is.na(pathological)) %>% 
  mutate(method = fct_lump_min(method, 10) %>% fct_infreq() %>% fct_rev())
```

```{r ffpe-hgmm, fig.width=7, fig.height=6, fig.cap="Number of FFPE and frozen section datasets from each current era technique in humans and mice healthy and pathological tissues; techniques used in fewer than 10 datasets are lumped into Other. LCM is only for curated LCM literature and does not include all search results in Chapter 6."}
ffpe_pathological <- ffpe_pathological %>% 
  mutate(pathological_label = case_when(
    pathological ~ "Pathological",
    TRUE ~ "Healthy"
  ))
ggplot(ffpe_pathological, aes(y = method, fill = is_ffpe)) +
  geom_bar() +
  facet_grid(pathological_label ~ species) +
  scale_x_continuous(expand = expansion(c(0, 0.05))) +
  scale_fill_discrete(name = "Is the\nsample\nFFPE") +
  labs(x = "Number of datasets", y = "Method")
```
...
```{r}
organs <- ms %>% 
  select(date_published:journal, method, country:year, species, organ,
         pathological) %>% 
  filter(str_detect(method, "GeoMX")) %>% 
  distinct()
```
...
```{r dsp-organs, fig.width=8, fig.height=6, out.width="100%", fig.cap="Number of GeoMX DSP or WTA studies for healthy and pathological human organs. Male is shown here because there are studies for the prostate but not for female specific organs."}
hg_h + hg_p +
...
```{r sf-plus, fig.cap="Schematic of seqFISH with pseudocolors.", out.width="100%"}
...
```{r smfish-lm-gene, fig.width=9, fig.height=6, fig.cap="(ref:smfish-lm-cap)", out.width="100%"}
ggplot(smfish, aes(date_published, n_genes)) +
...
```{r, results='markup'}
summary(lm(log(n_genes) ~ date_published, data = smfish))
```
...
```{r smfish-lm-cell, fig.width=9, fig.height=6, fig.cap="Total number of cells per study profiled by smFISH based techniques over time.", out.width="100%"}
...
smfish <- smfish %>% 
  mutate(method2 = case_when(
    ...
    method == "Molecular Cartography" ~ "Molecular Cartography",
    str_detect(method, "HybISS") ~ "HybISS",
    TRUE ~ "Other"
  ))
smfish_pubs <- get_pubs_df(smfish, other_cols = "method2")
```

```{r smfish-trend, fig.width=6, fig.height=6, fig.cap="Number of publications over time, broken down by technique type. Preprints are included, and the gray histogram in the background is the overall trend of all smFISH based techniques. Bin width is 90 days."}
...
```{r}
smfish_methods_cnt <- smfish_methods_cnt %>% 
  mutate(y_use = factor(n, levels = as.character(rev(seq_len(max(n))))))
```
...
ggplot(smfish_methods_cnt, aes(y_use)) +
   geom_bar() +
...
```{r smfish-map, fig.width=10, fig.height=6.5, fig.cap="(ref:smfish-map-cap)", out.width="100%"}
...
```{r smfish-langs, fig.width=6, fig.height=4, fig.cap="Number of publications using smFISH based techniques that used each of the 5 most common programming languages. Each icon stands for 10 publications."}
...
tab2 <- tibble(Technique = c("HCR-seqFISH", "seqFISH+", "MERFISH", 
               ...
               Pro = c("Relatively high efficiency (84%), fewer rounds of hybridization, error correction",
                       ...
                       "High efficiency (95%) with HD4 code, error correction, version 2 relatively fast, scalable, commercialized",
               ...
               Con = c("Lower efficiency than MERFISH, time consuming to re-hybridize probes to target after stripping",
                       ...
                       "Error correction not reported, amplicon takes up space and might drift away if not cross linked",
...
```{r cpal-fig, out.width="100%", fig.cap="Schematic of cPAL as used in ISS."}
...
```{r}
array <- read_metadata("NGS barcoding", update = TRUE)
...
```{r array-map, fig.height=13.75, fig.width=10, fig.cap="Cities and institutions using the two most popular technologies worldwide, the two methods used by the most institutions. Preprints are included.", out.width="100%"}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", n_top = 2, n_label = 10, ncol = 1)
```
...
array_methods <- array_methods |> 
  mutate(short_name = case_when(
    short_name == "Côte d’Azur" ~ "Cote d'Azur",
    TRUE ~ short_name
    ))
```

```{r array-europe, fig.height=10, fig.width=6, fig.cap="Cities and institutions using the two most popular tehcnologies in western Europe. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", 
            zoom = "europe",
            n_top = 2, n_label = 10, ncol = 1)
```
...
```{r array-trend, fig.width=6, fig.height=6, fig.cap="Number of publications over time, broken down by technique. The facets are ordered by total usage of the technique. Bin width is 90 days."}
pubs_per_year(array_methods, facet_by = "method", n_top = 2, sort_by = "count",
                      preprints = TRUE, binwidth = 90)
```
...
```{r species-pie-visium, fig.width=5, fig.height=4, fig.cap="Number of publication per species."}
...
```{r visium-organs, fig.width=8, fig.height=6, out.width="100%", fig.cap="(ref:hg-visium-cap)"}
hg_h + hg_p +
  plot_annotation(tag_levels = "A")
```
...
mm_p <- organs_mm_f %>% 
  ...
  scale_fill_distiller(palette = "Reds", direction = 1, 
                       name = "# publications\n(pathological)",
                       breaks = scales::breaks_extended(Q = c(1,2,4,5)))
```

```{r visium-organs-mm, fig.width=8, fig.height=6, out.width="100%", fig.cap="Number of Visium studies for healthy (A) and pathological (B) mouse organs. Female is shown here as there is a uterus study while there is no study on male specific organs in our database."}
mm_h + mm_p +
  plot_annotation(tag_levels = "A")
```
...
```{r array-us, fig.height=9, fig.width=6, fig.cap="Cities and institutions using the two most popular technologies around continental US. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", 
            zoom = "usa",
            n_top = 2, n_label = 10, ncol = 1)
```
...
```{r}
min_spot_size <- array %>% 
  select(method, spot_diameter_um, date_published) %>% 
...
min_spot_size <- min_spot_size %>% 
  mutate(drop = c(NA, diff(record_diam)),
         label = case_when(
           drop < 0 | is.na(drop) ~ method,
           TRUE ~ ""
         ))
```
...
```{r array-ne, fig.height=10, fig.width=6, fig.cap="Cities and institutions using the two most popular tehcnologies in Northeast Asia. Preprints are included."}
pubs_on_map(array_methods, city_gc = st_city_gc, facet_by = "method", 
            zoom = "ne_asia",
            n_top = 2, n_label = 10, ncol = 1)
```
