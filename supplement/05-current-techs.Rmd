...
ffpe2 <- ffpe %>% 
  mutate(method = fct_lump_min(method, 5) %>% fct_infreq() %>% fct_rev())
```

```{r ffpe, fig.width = 6, fig.height=4, fig.cap="Number of FFPE and frozen section datasets from each current era technique; techniques used in fewer than 5 datasets are lumped into Other. LCM is only for curated LCM literature and does not include all search results in Chapter 6."}
